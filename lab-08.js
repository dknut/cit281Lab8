...
fastify.get("/photos", (request, reply) => {
  ...
  fetch("https://jsonplaceholder.typicode.com/photos")
    .then((responseFromSite) => {
      return responseFromSite.json();
    })
    .then((responseasJson) => {
      reply
        .code(200)
...
fastify.get("/showphotos/:id", (request, reply) => {
  const { id = "" } = request.params;
  fetch(`https://jsonplaceholder.typicode.com/photos/${id}`)
    .then((responseFromSite) => {
      return responseFromSite.json();
    ...
    .then((responseasJson) => {
      reply
        ...
        .send(
          `<html><body><h1>${responseasJson.title}</h1><img src=${responseasJson.url} /></body></html>`
        );
    })
    .catch((err) => {
      reply
